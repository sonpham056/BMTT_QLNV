...
public class PnAuditHistoryAdmin extends JPanel {
	...
	private JCheckBox cbTurnOnOrOffSystemAudit;
	...
	public PnAuditHistoryAdmin() {
		...
		cbTurnOnOrOffSystemAudit.addMouseListener(new MouseAdapter() {
			...
			public void mouseReleased(MouseEvent e) {
				cbTurnOnOrOffSystemAuditClicked();
			}
		});
		cbTurnOnOrOffSystemAudit.setFont(new Font("Tahoma", Font.PLAIN, 20));
	...
	//=========================================================================================================================================
	private void cbTurnOnOrOffSystemAuditClicked() {
		try {
			if (cbTurnOnOrOffSystemAudit.isSelected()) {
				if (JOptionPane.showConfirmDialog(this, "Turn on system audit?", "confirm", JOptionPane.YES_NO_OPTION) == JOptionPane.YES_OPTION){
					SystemServices.turnOnOrOffAuditing(true);
					ViewBag.isAudit = true;
				} else {
					cbTurnOnOrOffSystemAudit.setSelected(false);
				}
			} else {
				if (JOptionPane.showConfirmDialog(this, "Turn off system audit?", "confirm", JOptionPane.YES_NO_OPTION) == JOptionPane.YES_OPTION){
					SystemServices.turnOnOrOffAuditing(false);
					ViewBag.isAudit = false;
				} else {
					cbTurnOnOrOffSystemAudit.setSelected(true);
				}
			}
		} catch (Exception e) {
	...
	private void loadTable() {
		try {
			...
			List<AuditHistory> list = AuditHistoryBUS.getAuditHistory("admin");
	...
	private void loadTableWithDates() {
		try {
			...
			List<AuditHistory> list = AuditHistoryBUS.getAuditHistoryWithDates(from, to, "admin");
			for (AuditHistory auditHistory : list) {
				model.addRow(new Object[] {
						auditHistory.getUser().getEmail(),
						auditHistory.getAudit().getType(),
	...
	private void loadAuditCheckbox() {
		if (ViewBag.isAudit) {
			cbTurnOnOrOffSystemAudit.setSelected(true);
		} else {
			cbTurnOnOrOffSystemAudit.setSelected(false);
